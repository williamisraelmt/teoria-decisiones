...
import {isUndefined, merge, cloneDeep, isNull} from 'lodash';

const REPORT_CODE = 'dailyEquipmentAudit';

@Component({
  selector: 'app-daily-equipment-audit',
  templateUrl: './daily-equipment-audit.component.html',
  styleUrls: ['./daily-equipment-audit.component.scss']
})
export class DailyEquipmentAuditComponent extends ReportTable implements OnInit, OnDestroy {
  ...
  ngOnInit(): void {
    this.report$ = this.reportService.get('dailyEquipmentAudit');
    this.settingsSub = this.settingsService.liveSettingsFromModule('DailyEquipmentAudit');
    this.settingsSub.pipe(first()).subscribe((v) => {
      this.form.patchValue(v, {emitEvent: false});
    });
    this.form.valueChanges.subscribe((v) => {
      this.settingsService.patchItems('DailyEquipmentAudit', v);
    });
